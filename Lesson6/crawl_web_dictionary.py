...
# Update for the crawl_web script to use dictionary
...
# lookup procedure to use dictionary
...
def add_to_index(index,keyword,url):
	# check if the keyword already exist using the 
...
# We want to crawl the seed web to make an
# index of all the words we can find
def crawl_web(seed):
	tocrawl = [seed]
	crawled = []
	# Initialize the empty dictionary for index
	index = {}
	# dictionary to store the links per page
	graph = {}
	while to:
		page = tocrawl.pop()
		if page not in crawled:
			# procedures get.page(), union() and
			# get_all_links() were copied from previous
			# lessons
			# https://www.udacity.com/course/viewer#!/c-cs101/l-48727569/e-48718374/m-48719196
			# Udacity 
			content = get.page(page)
			add_page_to_index(index,page,content)
			# create a variable to store all the links from the current
			# page
			outlinks = get_all_links(content)
			# actualize the graph of links
			graph[page] = outlinks
			union(tocrawl,outlinks)
			crawled.append(page)
	return index, graph

# algorithm to compute the rank of one page
